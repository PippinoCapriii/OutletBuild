...
public class ProdottoDAO implements ProdottoDAOInterfaccia{
	...
	public synchronized ArrayList<ProdottoBean> doFilter(String tipologia) throws SQLException {
		Connection connection = null;
		PreparedStatement preparedStatement = null;

		ArrayList<ProdottoBean> products = new ArrayList<ProdottoBean>();
		
		String selectSQL = "SELECT * FROM "+TABLE_NAME+" WHERE descrizione =? ";
	...
	public synchronized void doUpdate(ProdottoBean product) throws SQLException {
		...
		String updateSQL = "UPDATE " + ProdottoDAO.TABLE_NAME
				+ " SET NOME = ? , QUANTITA = ? , MARCA = ?, DESCRIZIONE = ?, PREZZO = ?, DATA_USCITA = ?, IVA = ?, IMMAGINE = ?, DESCRIZIONE_DETTAGLIATA = ?"
				+ " WHERE ID_PRODOTTO = ? ";
